...
const ROLE_LINKS: { slug: string; label: string }[] = [
  { slug: "elektroinstallateur-efz", label: "Elektroinstallateur EFZ" },
  { slug: "montage-elektriker-efz", label: "Montage-Elektriker EFZ" },
  { slug: "servicetechniker-elektro", label: "Servicetechniker Elektro" },
  { slug: "projektleiter-elektro", label: "Projektleiter Elektro" },
  { slug: "elektroplaner", label: "Elektroplaner" },
  { slug: "automatiker-efz", label: "Automatiker EFZ" },
  { slug: "betriebselektriker", label: "Betriebselektriker" },
  { slug: "photovoltaik-spezialist", label: "Photovoltaik-Spezialist" },
];

export async function generateStaticParams() {
  return ELEKTRIKER_CITIES.map((c) => ({ city: c.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { city: citySlug } = await params;
  const city = findElektrikerCity(citySlug);
  if (!city) return { title: "Elektriker Jobs" };

  const title = `Elektriker Jobs ${city.name} 2026 | Stellen für Elektriker in ${city.name}`;
  ...
  return {
    ...
    openGraph: {
      title: `${title} | elektrojob.ch`,
      description,
      url: `/elektriker-jobs/${city.slug}`,
      type: "website",
...
export default async function ElektrikerCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const city = findElektrikerCity(citySlug);
  if (!city) notFound();
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Startseite", item: SITE_URL },
      {
        "@type": "ListItem",
        position: 2,
        name: `Elektriker Jobs ${city.name}`,
        item: `${SITE_URL}/elektriker-jobs/${city.slug}`,
      },
    ],
  };
  ...
  const itemListSchema = {
    ...
    "@type": "ItemList",
    name: `Elektriker Jobs ${city.name}`,
    description: `Offene Elektriker Stellen in ${city.name}`,
  ...
  const FAQS = [
    ...
    },
    {
      ...
      answer: `Ein Elektroinstallateur EFZ verdient in ${city.name} typischerweise ${city.salaryBand} pro Jahr (12 × Monatslohn, ohne 13. ML und Boni). Berufseinsteiger starten am unteren Ende, mit fünf Jahren Erfahrung und Spezialisierungen wie Smart-Home, Photovoltaik oder KNX/DALI verschiebt sich der Lohn deutlich nach oben. Servicetechniker, Elektroplaner und Projektleiter Elektro verdienen in ${city.name} ähnlich oder leicht über den Installateurssaläre. Detaillierte Vergleichstabellen findest du auf der Lohnübersicht für Elektriker in der Schweiz.`,
    },
    {
    ...
    },
    {
      ...
      answer: `${city.name} ist gut mit ÖV erreichbar — die meisten Baustellen und Servicestützpunkte liegen entweder im Stadtgebiet (${city.districts.slice(0, 3).join(", ")}) oder in den Industrie- und Gewerbezonen am Stadtrand. Servicetechniker erhalten meist ein Geschäftsfahrzeug und starten direkt von zu Hause. Wer als Elektroinstallateur auf wechselnden Baustellen arbeitet, fährt typischerweise ab Werkstatt — Pendelweg dorthin sollte unter 30 Minuten liegen, sonst zahlt der Arbeitgeber gemäss GAV Wegspesen oder Zulagen. Die Pendlerorte ${city.commuterTowns.join(", ")} sind klassische Wohnorte für ${city.name}-Elektriker.`,
    },
  ];
      ...
      <main className="bg-white">
        <section className="bg-primary/5 border-b">
          <div className="container mx-auto px-4 sm:px-6 py-10 sm:py-14 max-w-5xl">
            <nav className="text-sm text-slate-500 mb-3" aria-label="Breadcrumb">
              ...
              <span className="text-slate-700">Elektriker Jobs {city.name}</span>
            </nav>
            <h1 className="text-3xl sm:text-5xl font-black text-slate-900 leading-tight mb-4">
              Elektriker Jobs <span className="text-primary">{city.name}</span>
            </h1>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
          <h2 className="text-2xl font-bold text-slate-900 mb-4">
            Offene Elektriker Stellen in {city.name}
          </h2>
        ...
        <section className="bg-slate-50 border-y">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-5xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-4">
              Elektriker Berufe in {city.name}
            </h2>
            <p className="text-slate-600 mb-5">
              Spezifische Elektriker-Profile in {city.name} und im Kanton {city.cantonAbbr}:
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
              {ROLE_LINKS.map((r) => (
                <Link
                  key={r.slug}
                  href={`/elektrojobs/${r.slug}/${city.cantonSlug}`}
                  className="rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 hover:border-primary/40 hover:text-primary transition"
                >
                  {r.label} — {city.name}
                </Link>
              ))}
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
          ...
          <p className="text-slate-600 mb-4">
            Pendelbereite Elektriker wohnen typischerweise in einem dieser Orte und arbeiten in {city.name}:
          </p>
          ...
          <p className="text-slate-500 text-sm mt-4">
            Stadtteile mit hoher Elektriker-Nachfrage: {city.districts.join(", ")}.
          </p>
        </section>
        ...
        <section className="bg-slate-50 border-t">
          <div className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
            <h2 className="text-2xl font-bold text-slate-900 mb-5">
              Häufig gestellte Fragen — Elektriker Jobs {city.name}
            </h2>
        ...
        <section className="container mx-auto px-4 sm:px-6 py-10 max-w-4xl">
          ...
          <div className="flex flex-wrap gap-2">
            {ELEKTRIKER_CITIES.filter((c) => c.slug !== city.slug).map((c) => (
              <Link
                key={c.slug}
                href={`/elektriker-jobs/${c.slug}`}
