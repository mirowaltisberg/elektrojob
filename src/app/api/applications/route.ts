...
export async function POST(request: Request) {
  ...
  try {
    ...
    const cv = formData.has("cv") ? getSingleFile(formData, "cv") : null;
    ...
    if (cv && !hasPdfMagic(buffer)) {
      return jsonError("Die PDF-Datei konnte nicht angenommen werden.", 400);
    }

    if (cv && /\.pdf$/i.test(filename) && hasDisallowedPdfFeatures(buffer)) {
      return jsonError(
    ...
    const now = new Date();
    const storagePath = cv ? `${now.getUTCFullYear()}/${String(now.getUTCMonth() + 1).padStart(2, "0")}/${randomUUID()}.${filename.split(".").pop()?.toLowerCase()}` : null;
    if (storagePath && cv) {
